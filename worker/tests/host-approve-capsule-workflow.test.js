...
test('host Time Capsule includes adaptive 3D walk review controls', async () => {
  ...
  assert.match(hostJs, /function loadSpatialLayoutReview/);
  ...
  assert.match(hostJs, /qs\("#refreshSpatialLayoutButton"\)\?\.addEventListener\("click", \(\) => loadSpatialLayoutReview\(\)\.then\(renderCapsule\)\)/);
  ...
  assert.match(hostJs, /\/host\/events\/\$\{encodeURIComponent\(eventId\)\}\/spatial-layouts\/review/);
  assert.doesNotMatch(hostJs, /\/host\/events\/\$\{encodeURIComponent\(eventId\)\}\/spatial-layouts\/draft/);
  ...
  assert.match(hostJs, /path\.endsWith\("\/spatial-layouts\/review"\)/);
  assert.doesNotMatch(hostJs, /path\.endsWith\("\/spatial-layouts\/draft"\)/);
  assert.match(hostJs, /path\.endsWith\("\/spatial-layouts\/generate"\)/);
